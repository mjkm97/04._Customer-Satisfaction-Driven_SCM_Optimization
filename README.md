...
여기 아래는 요청하신 내용을 **마크다운 형식 그대로** 정리한 버전이야. 바로 복붙해서 사용할 수 있어:
...
✔️ 정규화 → 비정규화 설계로 Spark / SQL 환경에서 **조인 최적화 및 처리 효율성 증대**  
✔️ 분석 및 모델링 시 **데이터 가독성 향상**, **확장성 및 유지보수 용이**
...
- 데이터 마트 설계부터 NLP, ML, 물류 최적화까지 **엔드투엔드 분석 역량**  
- 대규모 데이터 설계 및 컨테이너화 경험  
- 실무형 문제 해결력 및 전략적 사고
...
1. 실무 적용을 위한 물류센터 신설 비용 타당성 분석  
2. 리뷰 키워드 확장 (품질, 파손 등) 및 지역/상품별 분석  
